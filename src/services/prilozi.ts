...
const VLASNICI: Record<string, (tx: Tx, firmaId: string, id: string) => Promise<string | null>> = {
  ...
  Trosak: async (tx, firmaId, id) => {
    const r = await tx.$queryRaw<{ opis: string }[]>`SELECT opis FROM "Trosak" WHERE id = ${id}::uuid AND "firmaId" = ${firmaId}::uuid FOR UPDATE`;
    return r[0] ? `trošak ${r[0].opis}` : null;
  },
  ...
  ServisniNalog: async (tx, firmaId, id) => {
    const r = await tx.$queryRaw<
      { broj: string }[]
    >`SELECT broj FROM "ServisniNalog" WHERE id = ${id}::uuid AND "firmaId" = ${firmaId}::uuid FOR UPDATE`;
    return r[0] ? `servisni nalog ${r[0].broj}` : null;
  },
};
